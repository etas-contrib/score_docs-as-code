...
#           ╭───────────────────────────────────────────────────────────────╮
#           │  Tests for testcase_source_dirs scoping (docs(test_sources=)) │
#           ╰───────────────────────────────────────────────────────────────╯
...
def test_is_testcase_in_scope_empty_allow_list_is_unfiltered():
    """An empty allow-list disables filtering: everything is in scope."""
    assert xml_parser.is_testcase_in_scope("src/tests/e2e/basic/x.py", []) is True
    # Even a testcase without a `file` attribute is in scope when unfiltered.
    assert xml_parser.is_testcase_in_scope(None, []) is True
...
def test_is_testcase_in_scope_positive_match():
    """A file located under an allowed dir is in scope (positive match)."""
    assert (
        xml_parser.is_testcase_in_scope("src/tests/e2e/basic/x.py", ["src/tests/e2e"])
        is True
    )
...
def test_is_testcase_in_scope_not_under_any_allowed_dir():
    """A file outside every allowed dir is out of scope."""
    assert xml_parser.is_testcase_in_scope("src/other/x.py", ["src/tests/e2e"]) is False
...
def test_is_testcase_in_scope_none_file_with_allow_list():
    """With a non-empty allow-list a testcase without a `file` is out of scope."""
    assert xml_parser.is_testcase_in_scope(None, ["src/tests/e2e"]) is False
...
def test_is_testcase_in_scope_no_string_prefix_match():
    """Matching is component-wise, not a raw string-prefix match."""
    # "basic" starts with the allowed "ba" as a string, but not as a path
    # component, so this must NOT be considered in scope.
    assert (
        xml_parser.is_testcase_in_scope(
            "src/tests/e2e/basic/x.py", ["src/tests/e2e/ba"]
        )
        is False
    )
...
def test_parse_testcase_source_dirs():
    """`str(list)` config values are parsed into a list of paths."""
    assert xml_parser.parse_testcase_source_dirs("[]") == []
    assert xml_parser.parse_testcase_source_dirs("") == []
    assert xml_parser.parse_testcase_source_dirs('["a", "b/c"]') == ["a", "b/c"]
...
def test_parse_testcase_source_dirs_non_list_raises():
    """A malformed (non-list) config value fails loudly instead of downstream."""
    with pytest.raises(ValueError, match="must be a list"):
        xml_parser.parse_testcase_source_dirs('"not a list"')
