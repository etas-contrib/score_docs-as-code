...
def get_metadata_from_test_path(
    raw_filepath: Path, known_good_json: Path | None = None
) -> MetaData:
    """
    ...
    if repo_name != "local_repo" and known_good_json is not None:
...
def read_test_xml_file(
    ...
    known_good_json: Path | None = None,
...
def build_test_needs_from_files(
    ...
    Returns:
        - list[TestCaseNeed]
    """
    raw_known_good_json = getattr(app.config, "KNOWN_GOOD_JSON", "")
    known_good_json = (
        Path(raw_known_good_json)
        if isinstance(raw_known_good_json, str) and raw_known_good_json
        else None
    )
    ...
    for file in xml_paths:
        ...
        test_cases, tests_missing_all_props, tests_missing_some_props = (
            read_test_xml_file(file, allowed_dirs, known_good_json)
        )
